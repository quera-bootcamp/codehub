...
const ShopCards = () => {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                const response = await axios.get<Product[]>("http://localhost:5000/api/products/allproducts");
                ...
                <div key={item._id} className={'bg-default-600 dark:bg-default-900 rounded-lg w-96'}>
                    ...
                    <div className={'p-4 space-y-2'}>
                        <div className={'flex justify-between items-center font-primaryBold'}>
                            ...
                            <span className={'text-danger'}>{item.price.toLocaleString()} تومان</span>
                        </div>
                        <p className={`break-words whitespace-normal line-clamp-2 min-h-12 text-default-400`}>{item.description}</p>
